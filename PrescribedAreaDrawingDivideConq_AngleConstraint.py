...
def movePointsTowardsTargetWithAngularConstraint(prev_point, target_point, angle_in_degree, angle_neighbour_points,
                                                 all_neighbour_points):

    if target_point.distance(prev_point.loc) < 0.01:
        return prev_point.loc

    return_point = target_point

    for perc in range(5, 100, 5):
        dist_mov = float(perc / 100)

        is_lower_than_min_angle = False
        is_concave = False

        for points in angle_neighbour_points:
            for p in points:
                if p.loc.x == -1 and p.loc.y == -1:
                    p.loc = return_point

            l1 = Line(points[0].loc, points[1].loc)
            l2 = Line(points[2].loc, points[1].loc)
            deg = m.degrees(sympify(str(l1.angle_between(l2))))
            if deg < angle_in_degree:
                is_lower_than_min_angle = True
                break

        if not is_lower_than_min_angle:
            for points in all_neighbour_points:
                pol = Polygon(points[0].loc, points[1].loc, points[2].loc, return_point)
                if not pol.is_convex():
                    is_concave = True
                    break

        if is_lower_than_min_angle or is_concave:
            line1 = Line(target_point, prev_point.loc)
            return_point = line1.arbitrary_point(str(dist_mov))
        else:
            return return_point
    ...
    for p in range(len(points)):
        ...
        if (nodes[i][j].movable == False):
            ...
            if checkSignVal_1 >= 0 and checkSignVal_2 >= 0 and boundary_node_movement:
                ...
                p_empty = node(str(-1) + "" + str(-1), Point(-1, -1))
                val = movePointsTowardsTargetWithAngularConstraint(p_middle, val, min_angle_in_degree,
                                                   [(p_right, p_empty, p_bottom), (p_bottom_right, p_right, p_empty),
                                                    (p_right, p_empty, p_top), (p_top_right, p_right, p_empty)],
                                                   [(p_bottom, p_bottom_right, p_right),
                                                    (p_right, p_top_right, p_top)])
                nodes[i][j].loc = val
            ...
            if checkSignVal_1 <= 0 and checkSignVal_2 <= 0 and boundary_node_movement:
                ...
                p_empty = node(str(-1) + "" + str(-1), Point(-1, -1))
                val = movePointsTowardsTargetWithAngularConstraint(p_middle, val, min_angle_in_degree,
                                           [(p_top, p_empty, p_left), (p_empty, p_left, p_top_left),
                                            (p_bottom, p_empty, p_left), (p_empty, p_left, p_bottom_left)],
                                           [(p_top, p_top_left, p_left),
                                            (p_left, p_bottom_left, p_bottom)])
            ...
            if checkSignVal_1 >= 0 and checkSignVal_2 >= 0 and boundary_node_movement:
                ...
                p_empty = node(str(-1) + "" + str(-1), Point(-1, -1))
                val = movePointsTowardsTargetWithAngularConstraint(p_middle, val, min_angle_in_degree,
                                               [(p_top, p_empty, p_left), (p_left, p_top, p_empty),
                                                (p_right, p_empty, p_top), (p_empty, p_top, p_top_right)],
                                               [(p_top, p_top_left, p_left),
                                                (p_right, p_top_right, p_top)])
            ...
            if checkSignVal_1 <= 0 and checkSignVal_2 <= 0 and boundary_node_movement:
                ...
                val = movePointsTowardsTargetWithAngularConstraint(p_middle, val, min_angle_in_degree,
                                               [(p_left, p_empty, p_bottom), (p_empty, p_bottom, p_bottom_left),
                                                (p_bottom, p_empty, p_right), (p_empty, p_bottom, p_bottom_right)],
                                               [(p_left, p_bottom_left, p_bottom),
                                                (p_bottom, p_bottom_right, p_right)])
            ...
            if (val[0] == -1 and val[1] == -1):
                ...
                p_empty = node(str(-1) + "" + str(-1), Point(-1, -1))
                val = movePointsTowardsTargetWithAngularConstraint(p_middle, val, min_angle_in_degree,
                     [(p_top, p_empty, p_left), (p_top_left, p_top, p_empty), (p_empty, p_left, p_top_left),
                      (p_left, p_empty, p_bottom), (p_bottom_left, p_left, p_empty), (p_empty, p_bottom, p_bottom_left),
                      (p_bottom, p_empty, p_right), (p_bottom_right, p_bottom, p_empty), (p_empty, p_right, p_bottom_right),
                      (p_right, p_empty, p_top), (p_top_right, p_right, p_empty), (p_empty, p_top, p_top_right)],
                     [(p_top, p_top_left, p_left),
                      (p_left, p_bottom_left, p_bottom),
                      (p_bottom, p_bottom_right, p_right),
                      (p_right, p_top_right, p_top)])
...
def pointDPT_based_on_horizontal_points(h_grid_assigned, cpu_unassigned, values, max_heuristic, grid_count_horizontal,
                                        grid_count_vertical):
    final_array_first = []
    final_array_second = []
    grid_index = 0
    ...
    for th in range(cpu_unassigned):
        ...
        if boundary_index < total_h_grid:
            for k in range(grid_count_vertical + 1):
                ...
                # Bottom Right
                val4 = 0 if (boundary_index == grid_count_horizontal or k == grid_count_vertical) else \
                values[boundary_index][k]
                max_neighbourhood_val = max(val1, val2, val3, val4)
...
if __name__ == "__main__":
    ...
    output_txt_file.write(
        "Stage, Iteration, |UV-EV|/EV, UV/EV - 1, RMSE, MQE = (((|UV-EV|/EV) ** 2) ** 0.5)/N, Updated MQE = (((|UV-EV|/(UV+EV)) ** 2) ** 0.5)/N, Pre processing Time(sec), Iteration Time(sec)\n")
    ...
    for stg in range(stage_count):
        ...
        point_to_be_changed_array, point_to_between_thread = pointDPT_based_on_horizontal_points(
            grid_count_horizontal + 1,
            cpu_count, values,
            is_max_heiristic,
        ...
        for x in range(iteration):
            ...
            all_error_calc(values, nodes, grid_count_horizontal, grid_count_vertical, estimation_time,
                           output_img_filename, x + 1, stg, preprocessing_time)

            #poly_draw(output_img_filename, "_stage" + str(stg) + "_after" + str(x), output_image_size, nodes,
            #          grid_count_horizontal, grid_count_vertical)
